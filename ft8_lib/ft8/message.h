...
{
    // parallel arrays:
    // e.g. "K1ABC W9XYZ AB12" generates
    // types { FTX_FIELD_CALL_TO, FTX_FIELD_CALL_DE, FTX_FIELD_CALL_GRID" }
    // offsets { 0, 6, 12 }
...
/// Pack (encode) a text message, guessing which type to use and falling back on failure:
/// ftx_message_encode_std for message with callsign(s) (or for a CQ),
/// ftx_message_encode_nonstd if that fails because of a non-standard callsign,
/// or ftx_message_encode_free as fallback if that fails.
